...
namespace Encoder.Test
{
    public class EncoderTests
    {
        ...
        [TestCase(new int[] { 32, 4, 12, 78}, " $0~")]
        public void TestDecoder(int[] input, string result)
        {
            var decoder = new Encoder();

            Assert.AreEqual(result, decoder.Decode(input));
        }
        ...
        public void TestEncoder(string input, int[] result)
        {
            var encoder = new Encoder();

            var encodedValue = encoder.Encode(input);

            Assert.AreEqual(result, encodedValue);
        }
    }
}